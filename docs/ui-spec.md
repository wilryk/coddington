...
- **Degenerate geometries are legal and graceful.** *(Not built in 0.1.0: a 0° axicon half angle and a flat-solving Cassegrain are still rejected as validation errors. Descoped, not forgotten.)* Axicon half angle 0° is a flat disc mirror; a Cassegrain whose solve tends to R → ∞ is a flat fold mirror. Both trace exactly as flat mirrors, with a small info badge ("degenerate: tracing as flat mirror") — never a validation error. (Today's `gt=0` validators relax accordingly.)
...
- **Optical errors** are part of the heliostat design: **slope error (mrad)**, **specularity (mrad)**, **reflectance (%)**. They apply at **every fidelity**, not only Monte Carlo — slope and specularity broaden the spot (the cone backend folds them into its kernel, Monte Carlo perturbs per ray), reflectance scales collected power — and ride through SolTrace/SolarPILOT export. **Defaults are 0 / 0 / 90 %.**
